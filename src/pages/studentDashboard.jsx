  ...
  const getData = async () => {
    try {
      const resp = await Axios.get(
        "http://localhost:4050/api/v1/student/getprofile",
        {
          withCredentials: true,
        }
      );
      setdbData(resp.data.student);

      let x = 0;

      resp.data.student.allContracts.map((ele) => {
        x += ele.fundsRaised;
      });

      setTotalInv(x);
      setTotalContract(resp.data.student.allContracts.length);
      setConnections(resp.data.student.connections.length)
    } catch (e) {
      console.log(e);
    }
  };
  ...
  return (
    <div className="bg-primary h-full pb-8">
      <h1 className="font-bold text-[24px] pt-8 px-12">
        Good Afternoon, {dbData?.name}!
      </h1>

      <div className="flex flex-row flex-wrap gap-4 pt-8 px-12">
        <div className="">
          <Card
            text="Total Fundings Recieved"
            data={`$${totalInv}`}
            icon={
              <svg
